...
pub struct Cli {
    ...
    #[arg(long, default_value = "5", long_help = "\
    ...
    #[arg(long, default_value = "20", long_help = "\
    ...
    #[arg(long, default_value = "3", long_help = "\
    ...
    #[arg(long, default_value = "60", long_help = "\
...
pub enum InfraAction {
    ...
    #[command(long_about = "Removes all discovered services and dependencies from the database.\nThis cannot be undone — you will need to run `infra discover` again.")]
    Clear,
}
...
impl Cli {
    pub fn read_problem(&self) -> anyhow::Result<String> {
        match (&self.problem, &self.file) {
            ...
            _ => Err(anyhow::anyhow!(
                "Provide a problem via --problem or --file"
            )),
        }
    }
}
