...
impl WatchLoop {
    pub fn new(config: WatchConfig, cancellation: CancellationToken, json_mode: bool) -> Self {
        let checks: Vec<MonitorCheck> =
            config.checks.iter().map(|c| c.to_monitor_check()).collect();
        Self {
            checks,
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        if self.checks.is_empty() {
            eprintln!(
                "⚠ No checks configured. Use --watch-config or run `opcrew infra discover` first."
            );
            return Ok(());
        }
        ...
        loop {
            ...
            let healthy = results
                .iter()
                .filter(|r| r.status == CheckStatus::Healthy)
                .count();
            ...
            if healthy == total {
                ...
                if self.auto_fix && !critical_problems.is_empty() {
                    ...
                    if let Some(tx) = &self.problem_tx {
                        tracing::info!(
                            issues = critical_problems.len(),
                            "Triggering auto-fix pipeline"
                        );
                        println!(
                            "  >>> Triggering auto-fix for {} critical issues...",
                            critical_problems.len()
                        );
                        ...
                        tracing::info!(
                            issues = critical_problems.len(),
                            "Auto-fix: no pipeline connected (dry-run or missing config)"
                        );
